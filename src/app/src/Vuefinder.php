...
$aPath = explode('uploads://', $path);
$sFilename = '/usr/src/app/public/uploads/'.$aPath[1];

$stream = fopen($sFilename, 'r+');
if(0===filesize($sFilename)){
    $content = '';
}
else {
    $content = fread($stream, filesize($sFilename));
...
echo $content;die();
        ...
        $response->setCallback(function() use ($stream2) {
